...
/**
 * Installation sanity checks for the IHTS home page only.
 */

function ihts_sanity_die($title, $body_html) {
    error_log('IHTS sanity: '.$title);
...
if ( ! is_dir($tsugi_dir) ) {
    ihts_sanity_die(
        ...
        '<p>This copy of Internet History, Technology and Security expects a <code>tsugi</code> folder next to
        <code>index.php</code>, but that folder was not found.</p>
        <p>From your <code>ihts</code> directory, check out Tsugi:</p>
        <pre>cd ihts
...
$tsugi_markers = array('config-dist.php', 'lib/include/setup.php', 'index.php');
$missing = array();
...
if ( count($missing) > 0 ) {
    ihts_sanity_die(
        ...
        <p>From your <code>ihts</code> directory, refresh the Tsugi checkout:</p>
        <pre>cd ihts/tsugi
git pull
...
cp config-dist.php config.php</pre>
        <p>Edit <code>tsugi/config.php</code> and set at least <code>$CFG->pdo</code>,
        <code>$CFG->dbuser</code>, <code>$CFG->dbpass</code>, <code>$CFG->apphome</code>,
        <code>$wwwroot</code>, and <code>$CFG->adminpw</code>.</p>
        <p>Open <a href="'.htmlspecialchars($tsugi_url).'">'.htmlspecialchars($tsugi_url).'</a>
        for the Tsugi setup walkthrough.</p>
        <p>Installation help: <a href="http://www.tsugi.org/" target="_blank" rel="noopener noreferrer">tsugi.org</a></p>'
    );
}
